...
# from sklearn.model_selection import KFold, GroupKFold, LeaveOneOut, LeavePOut  # 交叉验证所需的子集划分方法
from sklearn.model_selection import ShuffleSplit  # 交叉验证所需的子集划分方法
...
for i in range(1, 8):
    ...
    if (i == 1):
        data_31 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_31 = data_31.T
        ratio_31 = []
        ...
        data_32 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_32 = data_32.T
        ratio_32 = []
        for j in range(p + 2, lenth + 1):
        ...
        data_33 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_33 = data_33.T
        ratio_33 = []
        for j in range(p + 2, lenth + 1):
        ...
        data_34 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_34 = data_34.T
        ratio_34 = []
        for j in range(p + 2, lenth + 1):
        ...
        data_35 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_35 = data_35.T
        ratio_35 = []
        for j in range(p + 2, lenth + 1):
        ...
        data_36 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_36 = data_36.T
        ratio_36 = []
        for j in range(p + 2, lenth + 1):
        ...
        data_37 = np.vstack((data_freq1.T, data_freq2.T,
                             data_ener1.T, data_ener2.T))
        data_37 = data_37.T
        ratio_37 = []
        for j in range(p + 2, lenth + 1):
...
x_32, y_32 = get_data(newdata_3[len(ratio_31):len(
    ratio_31) + len(ratio_32), :], ratio_32, time_step)
...
model.fit(train_x, train_y, validation_data=(val_x, val_y),
          epochs=100, batch_size=100, verbose=2, callbacks=[history])
